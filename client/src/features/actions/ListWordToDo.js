...
const ListWordToDo = ({ onNumChange }) => {
  ...
  if (error) return <Typography color="error">Error: {error.message}</Typography>; // טיפול בשגיאות
  ...
  return (
    ...
    >
      {itemCount === 0 ? (
        ...
        <>
          <Typography variant="h5">כמות הבחנים שעליך להפוך ל-ACTIVE: {itemCount}</Typography>
